...
/**
 * @（#）:MyRedisTokenStore.java
 * @description:
 *  RedisConnectionFactory保存token的时候会出现错误，这个时候需要自定义MyRedisTokenStore类，
 *  实现TokenStore。MyRedisTokenStore和RedisTokenStore代码差不多一样，只是把所有conn.set(…)
 *  都换成conn..stringCommands().set(…)，
 *  PasswordEncoder密码验证clientId的时候会报错，
 *  因为5.0新特性中需要在密码前方需要加上{Xxx}来判别。所以需要自定义一个类，
 *  重新BCryptPasswordEncoder的match方法。
 * @author: jsen 2018/8/27
 * @version: Version 1.0
 */
